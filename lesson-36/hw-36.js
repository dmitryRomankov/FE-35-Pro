...
А результатом этой функции должен быть отфильтрованный массив, где параметры 2 и 3 равны в объекте фильма. Например: передаем (films, 'title', 'Black Widow') и на выходе получаем фильм с id=1 если передаем (films, 'year', 2011) , то получаем фильм с id=2
*/
const movies = [
  {
    id: 1,
...
function getGenres(movies) {
  ...
  let uniqGenresSet = new Set(allGenresArray);
...
function getActors(movies) {
  ...
  return allActorsArray.filter((currActor, index) => {
    return allActorsArray.indexOf(currActor) === index;
  })
}
