...
let allGenre = movies.reduce((acc, curr) => {
  let res = acc.concat(curr.genre);
  return [...new Set(res)];
}, []);
console.log(allGenre);

// 2. Собрать в массив всех актеров всех фильмов (без повторения)
let allActors = movies.reduce((acc, curr) => {
  let res = acc.concat(curr.actors);
  return [...new Set(res)];
}, []);
console.log(allActors);

// 3. Отсортировать фильмы по рейтингу по убыванию
let sortedRates = movies.map((item) => {
  return { ...item };
});
...
function filterFilmsYear(arr, num) {
  let res = [];
  arr.reduce((acc, curr) => {
    return num === curr.year ? res.push(curr) : acc;
  }, []);
  return res;
}
console.log(filterFilmsYear(movies, 1977));
...
function filterFilms(arr, str) {
  let res = [];
  arr.reduce((acc, curr) => {
    return curr.title.toLowerCase().includes(str.toLowerCase())
      ? res.push(curr)
      : acc;
  }, []);
  return res;
}
console.log(filterFilms(movies, "half"));
...
function filterFilmsPlot(arr, str) {
  let res = [];
  arr.reduce((acc, curr) => {
    return curr.title.toLowerCase().includes(str.toLowerCase()) ||
      curr.plot.toLowerCase().includes(str.toLowerCase())
      ? res.push(curr)
      : acc;
  }, []);
  return res;
}
console.log(filterFilmsPlot(movies, "voldemort"));
...
function getFilm(arr, field, str) {
  let res = [];
  arr.reduce((acc, curr) => {
    return curr[field.toLowerCase()] === str ? res.push(curr) : acc;
  }, []);
  return res;
}
console.log(getFilm(movies, "YEAR", 2001));
